...
int Settings :: ReadFile(const string& file_name) {
	...
	try {
		...
		last_errm = "Błąd otwierania pliku " + file_name;
		last_errc = -1;
		goto blad;
	}
...
int Settings :: ReadBuf(const string& buf) {
	...
	try {
		...
		last_errm = "Błąd otwierania strumienia na buforze";
		last_errc = -1;
		goto blad;
	}
...
bool Settings :: _checkValid(void) {
	if( ! isValid() ) {
		last_errm = "Obiekt ustawień nie jest ważny";
		last_errc = -100;
		return false;
	}
	return true;
...
Settings Settings :: Extract(const string& path) const {
	...
	if( ! isValid() ) {
		s.last_errm = "Obiekt ustawień nie jest ważny";
		s.last_errc = -100;
		return s;
	}
	...
	istringstream  *ifs;
		...
		s.last_errm = "Błąd otwierania strumienia na buforze";
		s.last_errc = -1;
		return s;
	}
	...
	if( 0 == sym ) {
		s.last_errm = "Pusty plik lub niewłaściwa ścieżka (" + path + ")";
		s.last_errc = -4;
		return s;
	}
