...
bool AlanInstrGoto :: Relocate(int pc,AlanCode *code,string& error) {
	...
	if( refpc < 0 ) {
		error = "Odwołanie do nieistniejącej etykiety o nazwie " + label;
		return false;
	}
	dpc = refpc - pc;
	return true;
...
int AlanInstrWhile :: Execute(AlanState* stat) {
	AlanValue v;
	v = val->Eval(stat);
	ASSERT( v.type == TP_BOOL, "ALAN: Wartość warunku instrukcji WHILE nie jest typu BOOL" );
	if( v.GetBool() )
		return 1;
	else
		return dpc;
...
int AlanInstrIf :: Execute(AlanState* stat) {
	AlanValue v;
	v = val->Eval(stat);
	ASSERT( v.type == TP_BOOL, "ALAN: Wartość warunku instrukcji IF nie jest typu BOOL" );
	if( v.GetBool() )
		return 1;
	return dpc;
}
...
int AlanInstrPauseWhile :: Execute(AlanState* stat) {
	AlanValue v;
	v = val->Eval(stat);
	ASSERT( v.type == TP_BOOL, "ALAN: Wartość warunku instrukcji PAUSE WHILE nie jest typu BOOL" );
	if( v.GetBool() )
...
void AlanCode :: Reset(void) {
	...
	if( ! relocated ) {
		string error = "";
		if( ! Relocate(error) ) {
			ERROR("Błąd podczas relokowania kodu: " + error );
		}
	}
}
...
int AlanCode :: AddSpecLabel(char type,string& error) {
	...
	if( SpecialLabel::SLT_ELSE == type ) {
		...
		if( speclabs.size() <= 0 ) {
			error = "Jest ELSE, a nie ma pasującego IF";
			return -1;
		}
	...
	if( SpecialLabel::SLT_EIF == type ) {
		...
		if( speclabs.size() <= 0 ) {
			error = "Jest EIF, a nie ma pasującego IF lub ELSE";
			return -1;
		}
		SpecialLabel &sl = speclabs.top();
		if( SpecialLabel::SLT_IF != sl.type && SpecialLabel::SLT_ELSE != sl.type) {
			error = "Jest ELSE, a nie ma pasującego IF lub ELSE";
			return -1;
		}
		int refpc = sl.refpc;
	...
	if( SpecialLabel::SLT_NEXT == type ) {
		...
		if( speclabs.size() <= 0 ) {
			error = "Jest NEXT, a nie ma pasującego FOR";
			return -1;
		}
		SpecialLabel &sl = speclabs.top();
	...
	if( SpecialLabel::SLT_WEND == type ) {
		...
		if( speclabs.size() <= 0 ) {
			error = "Jest WEND, a nie ma pasującego WHILE";
			return -1;
		}
		SpecialLabel &sl = speclabs.top();
...
void AlanCode :: SetVarVal(const string& varname,bool v) {
	...
	ASSERT( TP_BOOL == var->type, "ALAN: Proba przypisania zmiennej typu " + TypeName(var->type) + " wartości typu BOOL");
	var->SetValue(v);
}
void AlanCode :: SetVarVal(const string& varname,int v) {
	AlanVariable *var = GetVar(varname);
	ASSERT( 0 != var, "ALAN: Zmienna " + varname + " nieistnieje" );
	ASSERT( TP_INT == var->type, "ALAN: Proba przypisania zmiennej typu " + TypeName(var->type) + " wartości typu INT");
	var->SetValue(v);
}
void AlanCode :: SetVarVal(const string& varname,float v) {
	AlanVariable *var = GetVar(varname);
	ASSERT( 0 != var, "ALAN: Zmienna " + varname + " nieistnieje" );
	ASSERT( TP_FLOAT == var->type, "ALAN: Proba przypisania zmiennej typu " + TypeName(var->type) + " wartości typu FLOAT");
	var->SetValue(v);
}
void AlanCode :: SetVarVal(const string& varname,const string& v) {
	AlanVariable *var = GetVar(varname);
	ASSERT( 0 != var, "ALAN: Zmienna " + varname + " nieistnieje" );
	ASSERT( TP_STRING == var->type, "ALAN: Proba przypisania zmiennej typu " + TypeName(var->type) + " wartości typu STRING");
	var->SetValue(v);
}
