...
bool AlanControlable::AlanGetVarB(const std::string& name) {
	ASSERT( channel != 0, "Obiekt nie jest powiązany z kanałem ALANa" );
	ASSERT( channel->code, "Kanał ALANa ma niezdefiniowany kod" );
...
AlanCode* AlanChannel :: _parseCode(const string& src) {
	...
	try {
		...
		ERROR("Błąd otwierania strumienia na buforze");
		return 0;
	}
