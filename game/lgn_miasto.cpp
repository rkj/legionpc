...
void ROZBUDOWA(aint MIASTO) {
	...
	for(I=0;I<=5;++I) {													//	   For I=0 To 5
		...
		GADGET(4+I*24,12,20,20,"buż",5,12,9,1,I+1);	//buzenie
		...
		if( HY>263 ) {														//	      If HY>263
			...
			if( MouseClick()==1 ) {									//	         If Mouse Click=1
				...
				if( STREFA>0 and STREFA<8 ) {					//	            If STREFA>0 and STREFA<8
					...
					if( STREFA == 7 ) {
						//buzenie
						GADGET(4+(STREFA-1)*24,12,20,20,"BUŻ",12,5,11,1,0);	//	               GADGET[4+(STREFA-1)*24,12,20,20,BB$,12,5,11,1,0]
						SZER=16;
						WYS=16;
						CENA=1500;
						CZAS=1;
					...
					if( PREV_STREFA>0 && PREV_STREFA != STREFA) {
						if( PREV_STREFA==7 ) {
							GADGET(4+(PREV_STREFA-1)*24,12,20,20,"buż",5,12,9,1,-1);
						} else {
