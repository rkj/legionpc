...
#ifndef __SETTINGS_SCANNER_HH__
#define __SETTINGS_SCANNER_HH__
...
#include <cstdlib>
#include <cstring>
#include "_settings_symbols.h"
  using namespace std;
	void ERRMSG(const string& msg);

#line 29 "settings_scanner.hh"
...
class SettingsScanner_base
{
  protected:
    bool onScanError (ScanException &ex) { return false; }
    bool onIOError (ScanException &ex) { return false; }
    bool wrap () { return false; }
    void commonTokenAction (SettingsToken &tok) {}
    void onInit () {}
    void onDestroy () {}
};

class SettingsScanner: public SettingsScanner_base
{
public:
  ...
  struct t_laCell
  {
    ...
    t_laCell(SettingsToken *token_, struct t_laCell *next_) : token(token_), next(next_) {}
    ~t_laCell() { delete token; }
  };
  
  SettingsScanner (std::istream *in_ = NULL) throw (ScanException)
      : _reject(false), 
  _bsz(8 * 1024), _pbsz(100)
  , _buffer(NULL), _more(false)
  {
    switchToStream(in_);
    lookahead = false;
    la0 = _laBegin = _laEnd = new struct t_laCell(new SettingsToken, NULL);
    la0->next = la0;
    switchToState(START);
    onInit();
  }
  ...
  class StreamState
  {
    ...
    StreamState (std::istream *inputStream_, char *_buffer_, int _bsz_,
      ...
      inputStream(inputStream_), _buffer(_buffer_), 
        _ch(_ch_), _start(_start_), _eob(_eob_), _bol(_bol_), _bsz(_bsz_), _pbsz(_pbsz_),
       _lta(_lta_), _eof(_eof_),
  ...
  SettingsToken* la (int k) throw (ScanException)
  {
    struct t_laCell *p, *q;
    for (p = la0; k >= 0; k--, q = p, p = p->next) {
      if (p == _laEnd) {
        if (_laEnd->next == _laBegin)
          _laEnd->next = new struct t_laCell(new SettingsToken(), _laBegin);
        _scan(_laEnd->token);
        _laEnd = _laEnd->next;
      }
    }
  ...
  SettingsToken* la () throw (ScanException)
  {
    if (la0 == _laEnd) {
      if (_laEnd->next == _laBegin)
        _laEnd->next = new struct t_laCell(new SettingsToken(), _laBegin);
...
protected: // the ugly stuff
  ...
  void _scan(SettingsToken *token) throw (ScanException);
  Position bPos, ePos;
  std::istream *inputStream;
...
}; // end class SettingsScanner


} //end namespace cppcc

#endif // ifndef __SETTINGS_SCANNER_HH__
