...
#ifndef __ALAN_SCANNER_HH__
#define __ALAN_SCANNER_HH__
...
public:
  ...
  struct t_laCell
  {
    ...
    t_laCell(AlanToken *token_, struct t_laCell *next_) : token(token_), next(next_) {}
    ~t_laCell() { delete token; }
  };
  
  AlanScanner (std::istream *in_ = NULL) throw (ScanException)
      : _reject(false), 
  ...
  , _buffer(NULL), _more(false)
  {
    switchToStream(in_);
    lookahead = false;
    la0 = _laBegin = _laEnd = new struct t_laCell(new AlanToken, NULL);
    la0->next = la0;
    switchToState(START);
    onInit();
  }
  ...
  AlanToken* la (int k) throw (ScanException)
  {
    struct t_laCell *p, *q;
    for (p = la0; k >= 0; k--, q = p, p = p->next) {
      if (p == _laEnd) {
        if (_laEnd->next == _laBegin)
          _laEnd->next = new struct t_laCell(new AlanToken(), _laBegin);
        _scan(_laEnd->token);
        _laEnd = _laEnd->next;
      }
    }
  ...
  AlanToken* la () throw (ScanException)
  {
    if (la0 == _laEnd) {
      if (_laEnd->next == _laBegin)
        _laEnd->next = new struct t_laCell(new AlanToken(), _laBegin);
...
protected: // the ugly stuff
  ...
  void _scan(AlanToken *token) throw (ScanException);
  Position bPos, ePos;
  std::istream *inputStream;
...
}; // end class AlanScanner


} //end namespace cppcc

#endif // ifndef __ALAN_SCANNER_HH__
